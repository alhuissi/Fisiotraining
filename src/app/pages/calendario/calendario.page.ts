...
export class CalendarioPage implements OnInit {
  ...
  ionViewWillEnter() {
    ...
    if (this.authService.userDetails()) {
      ...
      if (this.authService.whatRole() === 'admin') {
        this.authIsAdmin = true;
        //Carga el calendario completo si es admin, (es igual al de los kine)
          ...
          colSnap.forEach(snap => {
            ...
            if (event.idKine === this.userID) {
              ...
              if (event.realizada === 'no') {
                let x = new Date;
                //Este código cambia el color del evento según si es futuro o pasado
                /*
                if(event.startTime >= x){
                  event.eventColor = 'rgba(50, 200, 50, 0.737)';
                }
                else{
 ...
 */
  ...
  //Cuando evento seleccionado por Kine es un entrenamiento
  async presentAlertEntrenamiento(event) {

    let x = event.startTime;

    const alert = await this.alertCtrl.create({
      header: 'Sesión de Entrenamiento',
      message: '<p><strong>Cliente: </strong>' + event.nombreAlumno + '</p> <p><strong>Fecha: </strong>' + x.getDate() + '/' + Number(x.getMonth() + 1) + '/' + x.getFullYear() + ' a las ' + x.getHours() + ':' + x.getMinutes() + '</p>',
      buttons: [
        {
          text: 'Ok',
          role: 'cancel',
  ...
  //Cuando evento seleccionado por Kine en calendario es una solicitud (Title en Firebase = 'Solicitud'). Usado en onEventSelectedKine(event)
  //*Faltan opciones de sugerir otro kine u otro horario
  async presentAlertConfirmSolicitud() {
    ...
    return new Promise(async (resolve) => {
      ...
      const alert = await this.alertCtrl.create({
        header: '¿Aceptar solicitud de entrenamiento?',
        message: '<p><strong>Cliente: </strong>' + this.solService.getSolicitud().nombre + ' ' + this.solService.getSolicitud().apellido + '</p> <p><strong>Fecha: </strong>' + x.getDate() + '/0' + Number(x.getMonth() + 1) + '/' + x.getFullYear() + ' a las ' + x.getHours() + ':' + x.getMinutes() + '</p>',
        buttons: [
          {
            text: 'Cancelar',
            role: 'cancel',
            ...
            handler: () => {
              ...
              let ID = this.solService.getSolicitudUserID();
              let nameKine = this.solService.getSolicitud().kine;
              let idKine = this.solService.getSolicitud().kineID;
  ...
  async onEventSelectedKine(event) {
    if (event.title == 'Solicitud') {
      ...
      const confirmation = await this.presentAlertConfirmSolicitud();

      if (confirmation) {
        //Kine acepta solicitud
        //Se registra una nueva sesión en Firebase
        //Y se elimina la solicitud
        let eventCopy = {
          title: this.solService.getSolicitud().nombre,  //this.event.title,
          startTime: this.solService.getSolicitud().startTime,
          endTime: this.solService.getSolicitud().endTime,
          allDay: false,
          desc: 'Sesión de Kinesiología',
          nombreKine: this.solService.getSolicitud().kine,
          nombreAlumno: this.solService.getSolicitud().nombre + ' ' + this.solService.getSolicitud().apellido,
          idKine: this.solService.getSolicitud().kineID,
          idAlumno: this.solService.getSolicitud().idAlumno,
          id: this.solService.getSolicitud().id,
          fecha: this.solService.getSolicitud().fecha,
          fechaEmision: this.solService.getSolicitud().fechaEmision,
          realizada: 'no',
        }

        this.db.collection('sesiones').doc(eventCopy.id).set(eventCopy);
        await this.toastExitoAgregarSesion();
        console.log('Sesión Guardada');
        this.db.collection('solicitudes').doc(eventCopy.id).delete().then(function () {
          console.log("Solocitud eliminada de la base de datos");
        }).catch(function (error) {
          console.error("Error eliminando solicitud de la base de datos: ", error);
        });

        this.resetEvent();
      }
      else {
        //Kine rechaza solicitud

      }
    }
    else if(event.realizada == 'si') {
        ...
        docRef.get().toPromise().then(doc => {
          if (doc.exists) {
            let user: any = {
              ...
              nVecesEvaluado:doc.data().nVecesEvaluado,
            ...
            resolve();
            this.presentAlertEntrenamiento(event);
          } else {
  ...
  async onEventSelectedCliente(event) {
    ...
    const alert = await this.alertCtrl.create({
      header: 'Solicitud pendiente de aprobación',
      message: '<p><strong>Kinesiólogo: </strong>' + event.kine + '</p><p>Fecha: ' + start + '</p>',
      buttons: [
        {
          text: 'Eliminar',
          handler: () => {
            console.log('Eliminando la solicitud.');
            this.db.collection('solicitudes').doc(event.id).delete().then(function () {
              console.log("Solocitud eliminada de la base de datos");
            }).catch(function (error) {
              console.error("Error eliminando solicitud de la base de datos: ", error);
            });
    ...
    const alertEntrenamiento = await this.alertCtrl.create({
      ...
      message: '<p><strong>Kinesiólogo: </strong>' + event.nombreKine + '</p><p>Fecha: ' + start + '</p>',
      buttons: ['OK']
    });
    if (event.title == 'Solicitud') {
      await this.solService.setSolicitudFromID(event.id);
